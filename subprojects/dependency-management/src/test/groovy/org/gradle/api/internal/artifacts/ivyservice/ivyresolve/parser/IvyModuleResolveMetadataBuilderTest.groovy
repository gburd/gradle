...
class IvyModuleResolveMetadataBuilderTest extends Specification {
    ...
    def "adds correct artifact to meta-data"() {
        ...
        then:
        def artifacts = meta.build().asImmutable(attributesFactory, objectInstantiator).getConfiguration("runtime").artifacts
        artifacts.size() == 1
    ...
    def "can be added to metadata that already contains artifacts"() {
        ...
        then:
        def resolveMetaData = meta.build().asImmutable(attributesFactory, objectInstantiator)
