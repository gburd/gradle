...
class ModuleMetadataCacheEntry {
    ...
    protected ModuleComponentResolveMetadata configure(MutableModuleComponentResolveMetadata input, ImmutableAttributesFactory immutableAttributesFactory, NamedObjectInstantiator namedObjectInstantiator) {
        input.setChanging(isChanging);
        input.setSource(moduleSource);
        return input.asImmutable(immutableAttributesFactory, namedObjectInstantiator);
    }
}
